...
begin
  ...
  trImport.StartTransaction;     // Стартуем транзакцию
  ...
  try
     for i := 3 to LastRow do        //Начинаем проход с 4-й строки
        begin
        ...
        ln := '';
        Bd:=0.0;
        fL.DelimitedText := String(Sheet.readstr(i, 1));     // Считываем ФИО
      ...
      if fL.Count > 0 then     //Еси не было ФИО, то нечего писать
      Begin
        ExcImp.Close();
        ...
        if ExcImp.RecordCount = 0 then         //Поищем в базе данных, нет ли  такого уже
        Begin
        SGN :=  AnsiUpperCase(Trim(String(Sheet.readstr(i, 4))));
         if SGN = 'Ч' then  st := 1  else st := 0;    //Пол ребенка
        ...
        if ExcImp.RecordCount = 0 then         //Поищем в базе адресов
        Begin
        ...
        Begin
          ...
          ExcImp.ParamByName('ZAKLAD').AsString := Trim(Sheet.readstr(i, 5));     //Если нет передаем параметры процедуре добавления записи
          ExcImp.Open();
    ...
    qrymaster.Refresh;
    qryedu.Refresh;
    qryreg.Refresh;                                                    //когда все прошли, завершаем транзакцию
    Book.Free;
except
  if trImport.Active then                                               //если что-то не так, то откатываем
    Begin
       trImport.Rollback;
     End;
end;
...
try
   ...
   ExcImp.ParamByName('YEARS').AsString := xAddEduForm.edYear.text;                 //Если нет передаем параметры процедуре добавления записи
   ExcImp.Open();
   ExcImp.Close();
...
try
   ...
   ExcImp.ParamByName('CLASSNUM').AsInteger := StrToInt(xEditEduForm.edNumClass.Text);                  //Если нет передаем параметры процедуре добавления записи
   ExcImp.Open();
   ExcImp.Close();
