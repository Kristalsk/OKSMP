...
begin
  ...
  years := 'Число повних років станом на 01.09.'+ trim(Edit1.Text) + ' року:';
  qryReport.Close;
  qryReport.SQL.Text :=
    'select '+
  ...
  Book.load( PAnsiChar(AnsiString(xAppPath + 'Excel\шаблоны\shablon1.xls')));
//  Book.addSheet('111');
...
'select '+
  ...
  qryReport.open;
      ...
      Sheet.writeStr(60,14,PAnsiChar(AnsiString('Число повних років станом на 01.09.'+ Edit1.text+  ' року:')));
  ...
  pth := PAnsiChar(AnsiString(xAppPath+'Excel\отчеты\report1.xls'));
 Book.save(pth);
 ShellExecute(0, 'open', Pwchar(xAppPath+'Excel\отчеты\report1.xls') , nil, nil, SW_SHOW);
 Book.Free;
end;
